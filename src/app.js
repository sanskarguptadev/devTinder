...
app.patch("/user/:userId", async (req, res) => {
  try {
    ...
    if (data?.skills?.length > 10) {
      throw new Error("Skills can't have more than 10 skills");
    }

    const userData = { ...data };
    const userSkills = new Set(userData?.skills);
    const sanitzedSkills = { ...data, skills: [...userSkills] };

    await User.findByIdAndUpdate({ _id: userId }, sanitzedSkills, {
      runValidators: true,
    });
    res.status(200).send("Updated Successfully");
  } catch (err) {
    res.status(400).send("User not is Updated : " + err.message);
  }
});
